...
resource "oci_core_vnic_attachment" "vnic-1" {
  create_vnic_details {
    display_name  = "N3"
    subnet_id     = var.Subnet-N3
    private_ip = "192.168.10.98"
    defined_tags  = {}
  }
  display_name = "N3"
...
resource "local_file" "raemis-bootstrap-config" {
  ...
  NMS_IP="${var.NMS_IP}"
  SMF_IP="${var.SMF_IP}"
  SMF_API_USERNAME="${var.SMF_API_USERNAME}"
  SMF_API_PASSWORD="${var.SMF_API_PASSWORD}"
  UPF_SELECTION_DNN="${var.UPF_SELECTION_DNN}"
  UPF_N4_NET_DEV="${var.N4}"
  EOT
